...
use editor_state::{Change, Changes, TextSlice};
use ropey::Rope;

/// Edit op against a `LoroText` child container.
///
/// Mirror of `crdt::codec::TextOp` (architect repo,
...
pub fn changes_to_text_ops(doc_before: &Rope, changes: &Changes) -> Vec<TextOp> {
    ...
    for c in collected.into_iter().rev() {
        let pos = u32::try_from(doc_before.byte_to_char(c.from)).unwrap_or(u32::MAX);
        let end = u32::try_from(doc_before.byte_to_char(c.to)).unwrap_or(u32::MAX);
        if end > pos {
            ops.push(TextOp::Delete {
                pos,
                len: end.saturating_sub(pos),
            });
        }
...
pub fn remote_text_to_changes(old: &Rope, new: &str) -> Changes {
    ...
    Changes::replace(start..o_end, new.slice(start..n_end))
}
        ...
        for op in ops {
            match op {
                ...
                TextOp::Delete { pos, len } => {
                    let from = usize::try_from(*pos).unwrap_or(usize::MAX);
