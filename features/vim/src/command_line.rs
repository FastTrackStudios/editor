...
use editor_state::{Changes, EditorState, KeySpec, Selection, TextSlice, TransactionSpec};
...
impl CommandLineState {
    #[must_use]
    pub const fn new(kind: CmdKind) -> Self {
        Self {
            kind,
            buffer: String::new(),
        }
...
pub(crate) fn dispatch(
    ...
    match key.key.as_str() {
        "Enter" => {
            // `as_mut` above already established this is `Some`; fall back to
            // the stale-buffer recovery path rather than asserting it.
            let Some(cl) = vim.command_line.take() else {
                vim.mode = Mode::Normal;
                return None;
            };
            vim.mode = Mode::Normal;
...
fn substitute_command(state: &EditorState, cmd: &str) -> Option<TransactionSpec> {
    ...
    let mut parts = rest.after(sep.len_utf8()).splitn(3, sep);
    let pat = parts.next()?;
    ...
    while line_start < hi {
        let line_end = doc
            .slice(line_start..hi)
            .find('\n')
            .map_or(hi, |i| line_start.saturating_add(i));
        let line = doc.slice(line_start..line_end);
        for (off, _) in line.match_indices(pat) {
            let at = line_start.saturating_add(off);
            changes.push(editor_state::Change::replace(
                at..at.saturating_add(pat.len()),
                rep,
            ));
