...
/// Single entry point.
///
/// Reads `key` against the pending state in `vim`, possibly mutates it, and
/// returns a transaction spec to apply (or `None` if the key was consumed
/// for accumulation or is irrelevant in this mode).
pub fn handle_key(
    state: &EditorState,
    vim: &mut VimState,
    key: &KeySpec,
