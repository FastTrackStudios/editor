...
use editor_state::{Changes, EditorState, KeySpec, Range, Selection, TextSlice, TransactionSpec};
...
fn motion_target(state: &EditorState, motion: Motion, count: Option<usize>) -> usize {
    match motion {
        Motion::DocEnd => count.map_or_else(
            || motions::last_line_first_nonblank(state),
            |n| motions::nth_line_first_nonblank(state, n.saturating_sub(1)),
        ),
        Motion::DocStart => {
...
fn ordered_inclusive(
    ...
    if kind == MotionKind::Inclusive {
        ...
        if bytes.get(hi).is_some_and(|&b| b != b'\n') {
...
fn single_char_normal_command(
    ...
    match ch {
        ...
        'Y' => {
            ...
            let to = line_end.saturating_add(1).min(state.doc.len());
...
fn caret_after_line_delete(state: &EditorState, from: usize, to: usize) -> usize {
    let s = state.doc.to_string();
    let bytes = s.as_bytes();
    let at_eol_after = to >= bytes.len() || bytes.get(to) == Some(&b'\n');
...
fn operator_linewise_between(
    ...
    let hi = motions::line_end(state, hi_pos)
        .saturating_add(1)
        .min(state.doc.len());
...
fn finish_operator(
    ...
    if Operator::from_char(ch) == Some(op) {
        ...
        let to_inclusive = to.saturating_add(1).min(state.doc.len());
...
fn apply_find(
    ...
    vim.clear_pending();
    if vim.is_visual() {
        let anchor = vim.visual_anchor.unwrap_or_else(|| caret(state));
        return Some(
...
fn finish_pending_input(
    ...
    match input {
        ...
        MotionInput::Replace => {
            ...
            let s = state.doc.to_string();
            let replaced = s.slice(from..to).chars().count();
            vim.clear_pending();
            if replaced < count {
                return None;
            }
...
const fn motion_needs_input(motion: Motion) -> Option<MotionInput> {
    match motion {
...
fn dispatch_replace(
    ...
    Some(
        TransactionSpec::new()
            ...
            .selection(Selection::caret(from.saturating_add(ch.len_utf8()))),
    )
}
...
fn visual_range(state: &EditorState, vim: &VimState, force_linewise: bool) -> (usize, usize, bool) {
    ...
    if linewise {
        ...
        let end = motions::line_end(state, to)
            .saturating_add(1)
            .min(state.doc.len());
        (start, end, true)
    } else {
        let s = state.doc.to_string();
        let bytes = s.as_bytes();
        let hi = if bytes.get(to).is_some_and(|&b| b != b'\n') {
...
fn dispatch_visual(
    ...
    match ch {
        ...
        'J' => {
            ...
            let s = state.doc.to_string();
            let joins = s
                .slice(lo..hi.min(s.len()))
                .bytes()
                .filter(|&b| b == b'\n')
                .count()
                .max(1);
            return Some(join_lines(state, vim, lo, joins.saturating_add(1)));
        }
        _ => {}
    }
    ...
    if entry.linewise {
        if before {
            ...
            return TransactionSpec::new()
                ...
                .selection(Selection::caret(start.saturating_add(nb)));
        }
        ...
        let insert_at = end.saturating_add(1);
        let nb = text
            .len()
            .saturating_sub(text.trim_start_matches([' ', '\t']).len());
        TransactionSpec::new()
            .changes(Changes::insert(insert_at, text))
            .selection(Selection::caret(insert_at.saturating_add(nb)))
    } else {
        let p = if before {
            caret(state)
        } else {
        ...
        let new_caret = p.saturating_add(text.len()).saturating_sub(last_len);
        TransactionSpec::new()
...
fn search_word_under_caret(
    ...
    if start == end {
        ...
        let mut p = pos;
        while bytes.get(p).is_some_and(|&b| !is_word(b)) {
    ...
    let word = doc.slice(start..end).to_string();
...
pub(crate) fn jump_to(
    ...
    let whole_match_at = |i: usize| -> bool {
        let end = i.saturating_add(word.len());
        if end > bytes.len() || !doc.is_char_boundary(i) {
            return false;
        }
        if doc.slice(i..end) != word {
            return false;
        }
        if !whole_word {
            return true;
...
fn join_lines(state: &EditorState, vim: &mut VimState, at: usize, count: usize) -> TransactionSpec {
    ...
    for _ in 0..joins {
        ...
        let mut end = line_end.saturating_add(1);
...
fn visual_paste(state: &EditorState, vim: &mut VimState) -> Option<TransactionSpec> {
    let (lo, hi, _) = visual_range(state, vim, false);
    ...
    let caret_pos = if text.is_empty() {
        ...
        let last_len = text.chars().last().map_or(1, char::len_utf8);
        lo.saturating_add(text.len()).saturating_sub(last_len)
    };
    Some(
        TransactionSpec::new()
...
/// `x` — delete the character under the caret, yanking it to the register.
///
...
/// Normal-mode keys that switch to Visual mode or open a command line.
///
/// Split out of [`single_char_normal_command`]'s `match`, which had grown past
/// a screenful; the arms are unchanged.
