...
///
/// Untagged: the discriminator is field
...
pub fn encode(msg: &Message) -> Vec<u8> {
    let body = serde_json::to_vec(msg).unwrap_or_else(|_| {
        // Unreachable in practice: every payload in `Message` is already a
        // `serde_json::Value`, which always re-serializes. If it somehow did
        // fail, emitting a valid frame keeps the byte stream parseable for the
        // peer, whereas panicking here would take down the host application
        // over one malformed message.
        br#"{"jsonrpc":"2.0","method":"$/encodeFailed","params":null}"#.to_vec()
    });
    let len = body.len();
    let mut out = Vec::with_capacity(len.saturating_add(32));
...
///
/// Handles messages split across reads and
...
impl FrameDecoder {
    ...
    /// # Errors
    ///
    /// Returns [`DecodeError`] if the header block is not valid UTF-8,
    /// the `Content-Length` header is missing or not a valid number,
    /// or the message body is not valid JSON-RPC.
    ///
    ...
    pub fn try_next(&mut self) -> Result<Option<Message>, DecodeError> {
        ...
        let header = std::str::from_utf8(self.buf.get(..header_end).unwrap_or(&[]))
            .map_err(|_| DecodeError::HeaderNotUtf8)?;
        ...
        let msg = serde_json::from_slice(self.buf.get(body_start..body_end).unwrap_or(&[]))?;
...
/// channels.
///
/// This channel pair *is* the transport abstraction —
...
impl Transport {
    ...
    /// # Errors
    ///
    /// Returns an error if the child process fails to spawn, or if the
    /// spawned child is missing the stdin/stdout handles configured below.
    pub fn stdio(cmd: &str, args: &[&str], cwd: Option<&std::path::Path>) -> std::io::Result<Self> {
        ...
        // Both are configured `piped()` above, so these are present; treat a
        // missing handle as a spawn failure rather than asserting on it.
        let mut stdin = child
            .stdin
            .take()
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                decoder.feed(chunk.get(..n).unwrap_or(&[]));
                loop {
