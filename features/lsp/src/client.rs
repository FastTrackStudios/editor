...
///
/// A poisoned mutex only means some other task panicked while holding it; the
/// value behind it is still structurally intact. The two mutexes in this file
/// guard a pending-request table and a per-document version counter, where the
/// worst case from a mid-update panic is one stale entry. Turning that into a
/// second panic — inside whatever app embedded the editor — is strictly worse
/// than carrying on with the data as it stands.
...
/// [`LspClient::new`] returns.
///
/// Diagnostics get a typed variant (they are the crate's raison
/// d'être); everything else passes through raw for the host — or a
/// future hover/completion layer — to inspect.
#[derive(Debug)]
...
/// methods are async and safe to call from any task.
///
/// Dropping the client tears down the router and (for the stdio
/// backend) kills the child process.
...
impl LspClient {
    ...
    /// # Panics
    ///
    /// The router task panics if the internal pending-requests mutex
    /// is poisoned (a prior task holding it panicked).
    #[must_use]
                ...
                match msg {
                    Message::Response(resp) => {
                        let waiter = resp
                            .id
                            .as_i64()
                            .and_then(|id| lock(&router_pending).remove(&id));
    ...
    /// # Errors
    ///
    /// Returns [`Error::Closed`] if the transport is gone before the
    /// response arrives, [`Error::Server`] if the server reports a
    /// JSON-RPC error, and [`Error::Json`] if the params or result
    /// fail to (de)serialize.
    ...
    /// # Errors
    ///
    /// Returns [`Error::Closed`] if the transport is gone, or
    /// [`Error::Json`] if `params` fails to serialize.
    ...
    /// # Errors
    ///
    /// Propagates any [`Error`] from the underlying `initialize`
    /// request or `initialized` notification.
    ...
    /// # Errors
    ///
    /// Propagates any [`Error`] from the underlying `shutdown`
    /// request or `exit` notification.
    ...
    ///
    /// Returns [`Error::Closed`] if the transport is gone, or
    /// [`Error::Json`] if the notification params fail to serialize.
    ///
    /// # Panics
    ///
    /// Panics if the internal version-counters mutex is poisoned (a
    /// prior task holding it panicked).
    pub async fn did_open(&self, uri: Uri, language_id: &str, doc: &Doc) -> Result<(), Error> {
        lock(&self.versions).insert(uri.to_string(), 1);
    ...
    /// # Errors
    ///
    /// Returns [`Error::NotOpen`] if `uri` has no open document
    /// (no prior [`did_open`](Self::did_open)), [`Error::Closed`] if
    /// the transport is gone, or [`Error::Json`] if the notification
    /// params fail to serialize.
    ///
    /// # Panics
    ///
    /// Panics if the internal version-counters mutex is poisoned (a
    ...
    pub async fn did_close(&self, uri: &Uri) -> Result<(), Error> {
        lock(&self.versions).remove(&uri.to_string());
    ...
    pub fn version_of(&self, uri: &Uri) -> Option<i32> {
        lock(&self.versions).get(&uri.to_string()).copied()
    }
}
