//! Position conversion — editor byte offsets ↔ LSP UTF-16 positions.
//!
//! The correctness-critical seam of the whole crate: every diagnostic
//! range and every incremental `didChange` passes through here.
//!
...
pub fn position_to_byte(doc: &Doc, pos: Position) -> usize {
    ...
    let line_start = rope.line_to_byte(line);
    let line_end = rope.line_to_byte(line.saturating_add(1)); // one-past-the-end is valid
    let mut remaining = usize::try_from(pos.character).unwrap_or(usize::MAX);
