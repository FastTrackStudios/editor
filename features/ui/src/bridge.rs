...
fn handle_input(
    ...
    let new_doc_len = isize::try_from(cur.doc.len())
        .unwrap_or(isize::MAX)
        .saturating_add(
            changes
                .iter()
...
fn handle_copy_range(state: Signal<EditorState>, v: &serde_json::Value) {
    ...
    .unwrap_or(0);
    let to =
        usize::try_from(v.get("to").and_then(serde_json::Value::as_u64).unwrap_or(0)).unwrap_or(0);
    let cur = state.read();
    let doc = cur.doc.to_string();
    ...
    let script = format!(r"navigator.clipboard && navigator.clipboard.writeText(`{escaped}`);");
    let _ = document::eval(&script);
}
...
fn handle_prop_add(
    ...
    let new_text =
        editor_state::markdown::serialize_property(key, &editor_state::markdown::PropValue::Empty);
