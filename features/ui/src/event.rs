...
/// Apply a transaction to `state` and notify `sink`.
///
/// The public entry point for edits originating **outside** the
/// `<Editor>` subtree (e.g. a properties panel in a side sidebar that
/// mutates the same document).
///
