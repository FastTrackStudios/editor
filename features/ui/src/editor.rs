...
#![expect(
    ...
    reason = "DecorationSource's fast path compares fn addresses for prop-diff \
              only; the lint guards against codegen-unit splits our build does \
              not do, and a false positive here costs one redundant re-render"
)]
...
use dioxus::prelude::*;
use editor_state::{
    Changes, DecoratedRange, EditorState, KeySpec, Keymap, Range, Selection, TextSlice,
    TransactionSpec,
};
...
const fn widget_focused_dom() -> bool {
    false
}
...
#[cfg(not(target_arch = "wasm32"))]
#[must_use]
pub const fn coarse_pointer() -> bool {
    false
}
            ...
            #[cfg(not(feature = "native"))]
            {
                ...
                if is_visual_arrow_key {
                    let direction = match press.key.as_str() {
                        ...
                        "l" => ("forward", "character"),
                        "k" => ("backward", "line"),
                        // `is_visual_arrow_key` above already restricted this
                        // to h/j/k/l, so "j" is the only case left.
                        _ => ("forward", "line"),
                    };
...
fn modal_caret_decoration(
    ...
    if ch < rope.len_chars() && rope.char(ch) != '\n' {
        let end = rope.char_to_byte(ch.saturating_add(1));
...
/// large pastes too.
pub fn diff_text(old: &str, new: &str) -> Changes {
    let ob = old.as_bytes();
    let nb = new.as_bytes();
    let mut start = 0;
    ...
    // Walk backwards while the two buffers still agree. Each side indexes with
    // its OWN end — they have different lengths, which is the whole point of a
    // common-suffix scan. Bound the two offsets first: written inline, this
    // trips clippy's `suspicious_operation_groupings`, whose suggested "fix"
    // (index both with `o_end`) would be a real bug.
    while o_end > start && n_end > start {
        let o_last = o_end.saturating_sub(1);
        let n_last = n_end.saturating_sub(1);
        if ob.get(o_last) != nb.get(n_last) {
            break;
        }
        o_end = o_last;
        n_end = n_last;
    ...
    while o_end < ob.len() && !old.is_char_boundary(o_end) {
        o_end = o_end.saturating_add(1);
    }
    while n_end < nb.len() && !new.is_char_boundary(n_end) {
        n_end = n_end.saturating_add(1);
    }
    let inserted = new.slice(start..n_end);
