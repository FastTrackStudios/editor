...
impl Patch {
    fn element(tag: &str, attrs: Vec<(String, String)>, kids: Vec<Self>) -> Self {
        Self::Element {
            tag: tag.to_string(),
            attrs,
            kids,
...
///
/// Returns the *children* of the editor root — the patcher applies them to the
/// existing `<div data-editor-id>` mount element rather than replacing it.
#[must_use]
...
fn render_node(arena: &Arena, tile: TileId) -> Option<Patch> {
    ...
    match t.kind {
        ...
        TileKind::Line => Some(render_line(arena, t, tid, pos)),
        TileKind::Text => {
        ...
        TileKind::WidgetBuffer => Some(render_widget_buffer(tid, pos)),
        TileKind::Widget => {
...
///
/// Split out of [`render_node`]'s `match` to keep that dispatcher readable;
/// the body is unchanged.
fn render_line(arena: &Arena, t: &Tile, tid: u32, pos: usize) -> Patch {
    let mut kids = Vec::new();
    for &c in &t.children {
        if let Some(p) = render_node(arena, c) {
            kids.push(p);
...
/// The zero-width `<img>` anchors bracketing an inline point widget.
///
/// Split out of [`render_node`]'s `match`; the body is unchanged.
fn render_widget_buffer(tid: u32, pos: usize) -> Patch {
                ...
                match &kids[0] {
                    Patch::Element { tag, kids, .. } => {
                        ...
                        match &kids[0] {
                            ...
                            Patch::Element { .. } => panic!("expected Text"),
                        }
                    }
                    Patch::Text { .. } => panic!("expected span"),
                }
            }
            Patch::Text { .. } => panic!("expected div"),
        }
    }
    ...
    fn point_widget_renders_buffer_imgs_around_span() {
        ...
        for i in [1usize, 3] {
            match &kids[i] {
                ...
                Patch::Text { .. } => panic!("expected buffer img"),
            }
        }
    }
