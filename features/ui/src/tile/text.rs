...
///
/// A tile of any other kind carries no text, so this yields `""` rather than
/// asserting: the callers are render and patch paths, where the worst outcome
/// of a wrong kind is an empty run, and a panic would take the host down.
#[must_use]
pub const fn text_of(tile: &Tile) -> &str {
    match &tile.body {
...
mod tests {
    ...
    /// A non-Text tile used to panic here. It now yields `""`: `text_of` sits
    /// on the render and patch paths, where a wrong kind should cost an empty
    /// run, not the host process.
    #[test]
    fn text_of_is_empty_for_non_text() {
        let t = Tile {
            parent: None,
            children: Vec::new(),
            length: 0,
