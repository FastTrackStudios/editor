...
pub fn pos_at_start(arena: &Arena, tile: TileId) -> usize {
    ...
    while let Some(parent) = arena[current].parent {
        acc = acc.saturating_add(pos_before(arena, parent, current));
        current = parent;
    }
    acc
}
...
pub fn pos_at_end(arena: &Arena, tile: TileId) -> usize {
    pos_at_start(arena, tile).saturating_add(arena[tile].length)
}

/// Doc offset of `child` within `parent`. Mirrors CM6's
/// `Tile.posBefore` (`tile.ts:97-104`).
///
/// # Panics
///
/// If `child` is not among `parent`'s children — a caller bug, usually a
/// stale id — this returns the offset just past the last child, i.e. the end
/// of `parent`'s span. That keeps every caller's arithmetic in range instead
/// of asserting mid-render.
#[must_use]
pub fn pos_before(arena: &Arena, parent: TileId, child: TileId) -> usize {
    ...
    for &sib in &arena[parent].children {
        ...
        pos = pos
            .saturating_add(arena[sib].length)
            .saturating_add(usize::from(arena[sib].break_after()));
    }
    pos
}
...
pub fn pos_after(arena: &Arena, parent: TileId, child: TileId) -> usize {
    pos_before(arena, parent, child).saturating_add(arena[child].length)
}
