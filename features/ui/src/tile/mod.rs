//! The tile tree — a Rust port of `CodeMirror` 6's `view/src/tile.ts`.
//!
//! The tile tree is the editor's intermediate representation between the
//! document model (`editor-state`) and the DOM. Every visible piece of
...
impl Tile {
    ...
    /// `BreakAfter` flag).
    #[must_use]
    pub const fn break_after(&self) -> bool {
