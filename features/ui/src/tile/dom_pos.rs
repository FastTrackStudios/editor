...
pub fn pos_from_tile(arena: &Arena, tile: TileId, offset: usize) -> usize {
    pos_at_start(arena, tile).saturating_add(offset)
}
    ...
    loop {
        ...
        for (idx, &child) in children.iter().enumerate() {
            let child_len = arena
                .get(child)
                .length
                .saturating_add(usize::from(arena.get(child).break_after()));
            let next_acc = acc.saturating_add(child_len);
            ...
            if inside {
                chosen = Some(child);
                local = local.saturating_sub(acc);
                break;
            }
            // Last child + position at the tail: use it.
            let is_last_child = idx.saturating_add(1) == children.len();
            if is_last_child && local <= next_acc {
                chosen = Some(child);
                local = local.saturating_sub(acc);
                break;
            }
