...
use editor_state::{DecoratedRange, DecorationKind, TextSlice};
...
impl<'a> TileBuilder<'a> {
    const fn new(arena: &'a mut Arena, root: TileId) -> Self {
        Self {
            arena,
            root,
            cur_line: None,
    ...
    fn run(&mut self, text: &str, decorations: &[DecoratedRange]) {
        ...
        while self.pos < text.len() || event_idx < events.len() {
            ...
            while let Some(ev) = events.get(event_idx) {
                if ev.at >= self.pos {
                    break;
                }
                match &ev.kind {
            ...
            if let Some(replace_until) = active_replace_until {
                ...
                if until > self.pos {
                    self.emit_hidden(until.saturating_sub(self.pos));
                }
            ...
            if end > self.pos {
                self.emit_text_run(text.slice(self.pos..end), &active_marks);
    ...
    fn emit_text_run(&mut self, slice: &str, marks: &[MarkSpec]) {
        ...
        for (i, ch) in slice.char_indices() {
            if ch == '\n' {
                if i > local {
                    self.emit_text(slice.slice(local..i), marks);
                }
                ...
                local = i.saturating_add(1);
            }
        }
        if local < slice.len() {
            self.emit_text(slice.after(local), marks);
        }
    }
    ...
    fn bump_lengths_up(&mut self, tile: TileId, length: usize) {
        ...
        while let Some(p) = cur {
            ...
            let current = self.arena.get(p).length;
            self.arena.get_mut(p).length = current.saturating_add(length);
...
const fn flag(f: TileFlag) -> TileFlagSet {
            ...
            DecorationKind::Widget { html } => {
                out.push(Event {
                    ...
                    kind: EventKind::Widget {
                        length: d.to.saturating_sub(d.from),
                        html: html.clone(),
                    },
                });
            }
