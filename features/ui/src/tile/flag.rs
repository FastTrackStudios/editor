//! Tile bitflags.
//!
//! 1:1 port of CM6's `TileFlag` enum (`view/src/tile.ts:18-36`). We
//! use a hand-rolled bitflag type to keep the editor-view crate
//! dep-light; if the API grows large we can swap in the `bitflags`
//! crate.
...
impl TileFlag {
    ...
    pub const POINT_WIDGET: u16 = Self::Before.as_u16() | Self::After.as_u16();
    /// Mask for "any widget-related flag."
    pub const WIDGET: u16 = Self::Before.as_u16()
        | Self::After.as_u16()
        | Self::IncStart.as_u16()
        | Self::IncEnd.as_u16()
        | Self::Block.as_u16();

    /// Returns this flag's bit value.
...
impl TileFlagSet {
    ...
    #[must_use]
    pub const fn contains(self, flag: TileFlag) -> bool {
        (self.0 & flag.as_u16()) != 0
    }

    pub const fn insert(&mut self, flag: TileFlag) {
        self.0 |= flag.as_u16();
    }

    pub const fn remove(&mut self, flag: TileFlag) {
        self.0 &= !(flag.as_u16());
    }

    #[must_use]
    pub const fn intersects(self, mask: u16) -> bool {
        (self.0 & mask) != 0
    }
}
