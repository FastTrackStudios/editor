...
/// Build a [`TileKind::Widget`] tile.
///
/// `length` is the doc length the widget *replaces* — 0 for an inline
/// point widget (sits between characters), positive for a replace
/// widget that covers a doc range. `flags` carries the
...
/// Build a [`TileKind::WidgetBuffer`] tile.
///
/// These are tiny invisible `<img>` placeholders CM6 inserts adjacent
/// to non-editable widgets to work around browser cursor-bugs next to
/// uneditable inline content.
///
/// Mirrors `WidgetBufferTile` (`tile.ts:488-505`).
#[must_use]
pub const fn new_widget_buffer_tile(flags: TileFlagSet) -> Tile {
    Tile {
        parent: None,
        children: Vec::new(),
        length: 0,
...
#[must_use]
pub const fn is_point_widget(tile: &Tile) -> bool {
...
#[must_use]
pub const fn widget_covers(tile: &Tile, side_negative: bool) -> bool {
