...
pub fn detect_trigger(doc: &str, caret: usize) -> Option<(CompletionKind, usize, String)> {
    ...
    if let Some(i) = seg.rfind("[[") {
        let query = seg.after(i.saturating_add(2));
        if !query.contains("]]") {
            return Some((
                CompletionKind::Wikilink,
                line_start.saturating_add(i),
                query.to_string(),
            ));
        }
    }
    ...
    if i > 0 && bytes.get(i.saturating_sub(1)) == Some(&b'#') {
        ...
        if prev_ok {
            return Some((
                CompletionKind::Tag,
                line_start.saturating_add(hash),
                seg.after(i).to_string(),
            ));
        }
    }
    None
...
pub fn accept_candidate(
    ...
    let replacement = match comp.kind {
        CompletionKind::Wikilink => {
            if doc.after(caret).starts_with("]]") {
    ...
    let new_caret = comp.trigger_start.saturating_add(replacement.len());
    Some(
        TransactionSpec::new()
